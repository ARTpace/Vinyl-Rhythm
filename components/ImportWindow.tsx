...
interface ImportWindowProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: () => void; // 改为直接触发系统文件夹选择
  importedFolders: LibraryFolder[];
}

const ImportWindow: React.FC<ImportWindowProps> = ({ isOpen, onClose, onImport, importedFolders }) => {
            ...
            <div className="space-y-3">
              ...
              <div className="max-h-[200px] overflow-y-auto custom-scrollbar space-y-2 pr-2">
                {importedFolders.map(folder => (
                  <div key={folder.id} className="bg-white/5 rounded-2xl px-5 py-3 flex items-center justify-between border border-white/5">
