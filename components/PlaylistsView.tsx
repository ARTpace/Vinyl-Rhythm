...
const PlaylistsView: React.FC<PlaylistsViewProps> = ({ playlists, onSelectPlaylist, onPlayPlaylist, onCreatePlaylist, onImportPlaylist, onDeletePlaylist, displayConverter }) => {
  return (
    <div className="flex flex-col h-full bg-zinc-950/20 overflow-hidden animate-in fade-in duration-500">
      <header className="p-4 md:p-8 shrink-0 pointer-events-none">
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-8 pt-8 pb-32 custom-scrollbar">
        {playlists.length === 0 ? (
          <div className="h-64 flex flex-col items-center justify-center text-zinc-800 gap-4">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.3em]">No Playlists Found</p>
          </div>
        ) : (
          ...
          {playlists.map(playlist => (
            <div 
              key={playlist.id} 
              className="group text-center animate-in fade-in zoom-in-95 duration-500"
            >
              <div className="relative">
                <button
                  onClick={(e) => { e.stopPropagation(); onDeletePlaylist(playlist.id, playlist.name); }}
                  className="absolute -top-2 -right-2 w-7 h-7 rounded-full bg-black/60 backdrop-blur-md border border-white/10 text-zinc-400 hover:bg-red-500/20 hover:text-red-500 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100 z-20"
                  title="删除歌单"
                >
                ...
                <div 
                  ...
                  className="relative aspect-square bg-zinc-900 border border-white/5 rounded-3xl transition-all group-hover:scale-105 group-hover:shadow-[0_0_30px_rgba(0,0,0,0.8)] flex items-center justify-center overflow-hidden shadow-2xl mb-3 cursor-pointer"
                >
                  {playlist.coverUrl ? (
